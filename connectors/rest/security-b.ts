...
export const tenableManifest = defineRestConnector({
  ...
  actions: [
    ...
    { slug: 'list_scans', name: 'List scans', risk: 'read', path: '/scans',
      description: 'All configured scans with last run status and schedule.', params: {} },
    { slug: 'get_scan', name: 'Get scan details', risk: 'read', path: '/scans/{scan_id}',
      description: 'Full config and latest results summary for one scan.',
      params: { scan_id: { type: 'integer', description: 'Scan ID', required: true } } },
    { slug: 'get_scan_history', name: 'Get scan history', risk: 'read', path: '/scans/{scan_id}/history',
      description: 'Every past run of a scan (all of it — not just what the UI shows). Great for trend/historical analysis.',
      params: { scan_id: { type: 'integer', description: 'Scan ID', required: true } } },
    { slug: 'create_scan', name: 'Create scan', risk: 'write', method: 'POST', path: '/scans',
      description: 'Creates a new scan. Get a template UUID from list_scan_templates; targets is a comma-separated list of hosts/IPs.',
      wrapBody: b => ({ uuid: b.template_uuid, settings: { name: b.name, text_targets: b.targets, enabled: false } }),
      params: { template_uuid: { description: 'Scan template UUID (from list_scan_templates)', required: true }, name: { description: 'Scan name', required: true }, targets: { description: 'Comma-separated hosts/IPs/CIDRs', required: true } } },
    { slug: 'launch_scan', name: 'Launch scan', risk: 'write', method: 'POST', path: '/scans/{scan_id}/launch',
      description: 'Starts a scan now.',
      params: { scan_id: { type: 'integer', description: 'Scan ID', required: true } } },
    { slug: 'stop_scan', name: 'Stop scan', risk: 'write', method: 'POST', path: '/scans/{scan_id}/stop',
      description: 'Stops a running scan.',
      params: { scan_id: { type: 'integer', description: 'Scan ID', required: true } } },
    { slug: 'delete_scan', name: 'Delete scan', risk: 'destructive', method: 'DELETE', path: '/scans/{scan_id}',
      description: 'Permanently deletes a scan and its history. Confirm before using.',
      params: { scan_id: { type: 'integer', description: 'Scan ID', required: true } } },
    { slug: 'export_scan', name: 'Export scan results', risk: 'write', method: 'POST', path: '/scans/{scan_id}/export',
      description: 'Requests an export of a scan\'s full results (Nessus/CSV/PDF). Returns a file id to download.',
      wrapBody: b => ({ format: b.format ?? 'csv' }),
      params: { scan_id: { type: 'integer', description: 'Scan ID', required: true }, format: { description: 'nessus | csv | pdf (default csv)', enum: ['nessus', 'csv', 'pdf'] } } },
    { slug: 'list_scan_templates', name: 'List scan templates', risk: 'read', path: '/editor/scan/templates',
      description: 'Available scan templates with their UUIDs (needed to create a scan).', params: {} },
    { slug: 'list_policies', name: 'List scan policies', risk: 'read', path: '/policies',
      description: 'Saved scan policies.', params: {} },
    // Assets
    { slug: 'list_assets', name: 'List assets', risk: 'read', path: '/assets',
      description: 'Discovered assets with OS and last-seen.', params: {} },
    { slug: 'get_asset', name: 'Get asset', risk: 'read', path: '/assets/{asset_id}',
      description: 'Full detail for one asset (interfaces, tags, exposure).',
      params: { asset_id: { description: 'Asset UUID', required: true } } },
    { slug: 'list_asset_vulnerabilities', name: 'Vulnerabilities on an asset', risk: 'read', path: '/workbenches/assets/{asset_id}/vulnerabilities',
      description: 'Every vulnerability found on one asset.',
      params: { asset_id: { description: 'Asset UUID', required: true } } },
    { slug: 'bulk_delete_assets', name: 'Bulk delete assets', risk: 'destructive', method: 'POST', path: '/api/v2/assets/bulk-jobs/delete',
      description: 'Deletes assets matching a filter. Confirm before using.',
      wrapBody: b => ({ query: { field: 'ipv4', operator: 'eq', value: b.ipv4 } }),
      params: { ipv4: { description: 'IPv4 address of assets to delete', required: true } } },
    // Vulnerabilities & findings
    { slug: 'list_vulnerabilities', name: 'List vulnerabilities', risk: 'read', path: '/workbenches/vulnerabilities', staticQuery: { date_range: '0', 'filter.0.quality': 'gte', 'filter.0.filter': 'severity', 'filter.0.value': '2' },
      description: 'Current vulnerabilities across all assets, ranked by severity. date_range=0 means all time — the full picture, not a UI window.', params: {} },
    { slug: 'get_vulnerability_info', name: 'Get vulnerability (plugin) info', risk: 'read', path: '/workbenches/vulnerabilities/{plugin_id}/info',
      description: 'Detail on one vulnerability/plugin: description, solution, CVEs, CVSS.',
      params: { plugin_id: { type: 'integer', description: 'Plugin ID', required: true } } },
    { slug: 'export_vulnerabilities', name: 'Export all vulnerabilities', risk: 'write', method: 'POST', path: '/vulns/export',
      description: 'Requests a bulk export of ALL vulnerability findings since a point in time (built for pulling full history the UI won\'t show). num_assets: chunk size.',
      wrapBody: b => ({ num_assets: Number(b.num_assets ?? 500), filters: b.since ? { since: Number(b.since) } : {} }),
      params: { num_assets: { type: 'integer', description: 'Assets per export chunk (default 500)' }, since: { type: 'integer', description: 'Unix timestamp — export findings updated since (optional; omit for everything)' } } },
    // Account
    { slug: 'get_license_info', name: 'Get license info', risk: 'read', path: '/api/v3/access-control/license',
      description: 'License details: asset entitlement, usage, and expiration.', params: {} },
    { slug: 'list_users', name: 'List users', risk: 'read', path: '/users',
      description: 'Tenable.io users and roles.', params: {} },
    { slug: 'list_scanners', name: 'List scanners', risk: 'read', path: '/scanners',
      description: 'Available scanners and their status.', params: {} },
    { slug: 'list_tags', name: 'List asset tags', risk: 'read', path: '/tags/values',
      description: 'Asset tag categories and values.', params: {} },
  ],
})
