...
import { AlertTriangle, Building2, Users, Plug, Activity, TrendingDown, ToggleLeft, ToggleRight } from 'lucide-react'
...
export default async function AnalysisPage() {
  ...
  const [
    ...
    admin.from('skill_runs').select('workspace_id, status').gte('started_at', monthStart),
    admin.from('connector_overrides').select('slug, disabled, disabled_reason'),
  ])

  // Per-workspace aggregations
  ...
  for (const c of allConnections ?? []) {
    connMap[c.workspace_id] = (connMap[c.workspace_id] ?? 0) + 1
  }
  const callMap: Record<string, number> = {}
  for (const a of auditMonth ?? []) {
    callMap[a.workspace_id] = (callMap[a.workspace_id] ?? 0) + 1
  }
  const skillMap: Record<string, number> = {}
  for (const s of skillRunsMonth ?? []) {
    skillMap[s.workspace_id] = (skillMap[s.workspace_id] ?? 0) + 1
  }
  ...
  const wsStats = (workspaces ?? []).map(ws => ({
    ...
    skillRunsThisMonth: skillMap[ws.id] ?? 0,
  }))
  ...
  for (const o of disabledOverrides ?? []) {
    overrideMap[o.slug] = { disabled: o.disabled, disabled_reason: o.disabled_reason }
  }

  // Only show connectors that are in the catalog
  ...
  return (
    <div className="p-8 space-y-10 max-w-6xl">
      <div>
        <h1 className="text-2xl font-bold">Platform Analysis</h1>
        <p className="text-muted-foreground mt-1 text-sm">Usage analytics, alerts, and connector management for all workspaces.</p>
      </div>
      ...
      {/* No-usage workspaces with connections */}
      {noUsage.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-center gap-2">
      ...
      <section className="space-y-3">
        ...
        <div className="rounded-xl border bg-card overflow-hidden">
          <div className="grid grid-cols-[1fr,80px,80px,100px,80px] gap-x-4 px-5 py-2.5 border-b bg-muted/30 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <span className="text-right">Calls (mo)</span>
          ...
          <div className="divide-y divide-border/50">
            {wsStats.map(ws => (
              <div key={ws.id} className="grid grid-cols-[1fr,80px,80px,100px,80px] gap-x-4 px-5 py-3 items-center">
                ...
                <p className={`text-sm text-right font-medium ${ws.callsThisMonth > HIGH_THRESHOLD ? 'text-amber-400' : ''}`}>
                  {ws.callsThisMonth.toLocaleString()}
                </p>
              ...
              <div key={c.slug} className="grid grid-cols-[1fr,120px,80px] gap-x-4 px-5 py-2.5 items-center">
                <div className="min-w-0">
                  ...
                  {c.disabled && c.disabled_reason && (
                    <p className="text-[11px] text-muted-foreground truncate">{c.disabled_reason}</p>
                  )}
                </div>
                <p className="text-xs text-muted-foreground">{c.category}</p>
                <div className="flex justify-end">
                  <ConnectorToggle slug={c.slug} name={c.name} disabled={c.disabled} />
                </div>
              </div>
            ))}
          </div>
