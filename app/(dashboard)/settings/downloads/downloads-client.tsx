...
import { Download, Copy, Check, Package, ShieldCheck, Terminal } from 'lucide-react'
...
export function DownloadsClient({ company, licenseExpiresAt, releases }: {
  company: string
  ...
  const expiryNote = licenseExpiresAt
    ? `Licensed to ${company} until ${new Date(licenseExpiresAt).toLocaleDateString()}.`
    : `Licensed to ${company}.`

  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">{expiryNote}</p>
