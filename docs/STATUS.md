...
- **Auth/data:** Supabase (migrations through **054** applied). Prod and dev are now **separate
  projects**; Vercel Preview runs against the dev one.
...
- ~~**Shared dev/prod database**~~ ✅ **SPLIT 2026-08-05** — a second Supabase project
  ("OrbitAPI Dev", ref `cijajlgpuykjjsdqjcmf`) now holds all 54 migrations, and Vercel
  **Preview/Development** point at it. Production's env vars were not touched. Preview has its
  own `CRON_SECRET` / `SIGNUP_INVITE_CODE` rather than reusing prod's.
  **Rehearse every new migration against dev first:**
  `NEXT_PUBLIC_SUPABASE_URL=https://cijajlgpuykjjsdqjcmf.supabase.co npm run db:up`
  ⚠️ Both free project slots are now used, and the dev project pauses after ~7 days idle.
...
- **Secrets in transcript** — Vercel token + Supabase/Anthropic keys were pasted during setup,
  plus the `SUPABASE_ACCESS_TOKEN` pasted 2026-07-03, and a `VERCEL_TOKEN` + Resend send-key
  pasted 2026-08-05. ⚠️ Still open — rotation is account-level.
...
4. ~~**Split dev/prod Supabase**~~ ✅ done 2026-08-05.
